...
SELECT ps.policy_type_code, COUNT(*) AS NO_OF_POLICIES
...
AND u.user_id NOT IN (
    ...
    JOIN policy_sub_types ps2 ON up2.policy_type_id = ps2.policy_type_id
    JOIN ref_policy_types rp2 ON ps2.policy_type_code = rp2.policy_type_code
    WHERE rp2.policy_type_name = 'home'
);

SELECT TOP 1 ps.policy_type_code, rp.policy_type_name
FROM user_policies up
JOIN policy_sub_types ps ON up.policy_type_id = ps.policy_type_id
JOIN ref_policy_types rp ON ps.policy_type_code = rp.policy_type_code
GROUP BY ps.policy_type_code, rp.policy_type_name
ORDER BY COUNT(*) DESC;
...
SELECT rp.policy_type_code, rp.policy_type_name,
       ps.policy_type_id, up.user_id, up.policy_no
FROM user_policies up
JOIN policy_sub_types ps ON up.policy_type_id = ps.policy_type_id
JOIN ref_policy_types rp ON ps.policy_type_code = rp.policy_type_code
WHERE DATEADD(YEAR, ps.maturityperiod, up.date_registered)
      BETWEEN '2013-08-01' AND '2013-08-31';

SELECT rp.policy_type_code, rp.policy_type_name, ps.policy_type_id
FROM policy_sub_types ps
JOIN ref_policy_types rp ON ps.policy_type_code = rp.policy_type_code
JOIN user_policies up ON ps.policy_type_id = up.policy_type_id
JOIN policy_payments pp ON up.policy_no = pp.policy_no
GROUP BY rp.policy_type_code, rp.policy_type_name, ps.policy_type_id, ps.maturityamount
HAVING ps.maturityamount = 2 * SUM(pp.amount);
...
SELECT up.user_id, up.policy_no,
       ps.maturityamount - ISNULL(SUM(pp.amount),0) AS balance_amount
FROM user_policies up
JOIN policy_sub_types ps ON up.policy_type_id = ps.policy_type_id
LEFT JOIN policy_payments pp ON up.policy_no = pp.policy_no
GROUP BY up.user_id, up.policy_no, ps.maturityamount;

SELECT up.user_id, up.policy_no,
       ps.yearsofpayements - COUNT(pp.receipno) AS BALANCE_YEARS
FROM user_policies up
JOIN policy_sub_types ps ON up.policy_type_id = ps.policy_type_id
LEFT JOIN policy_payments pp ON up.policy_no = pp.policy_no
GROUP BY up.user_id, up.policy_no, ps.yearsofpayements;
